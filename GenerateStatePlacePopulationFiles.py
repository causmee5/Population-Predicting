...
for year in years.index:
    ...
    #stateData = pd.read_csv(f"{years.iloc[year]['Year']}StatePopulation.csv", dtype=dtype_dict)
    for entry in stateData.index:
        ...
        query = years.iloc[year]['Place_Query'] + stateData.iloc[entry]['state'] + api_key
        #print(query)
